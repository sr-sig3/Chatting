...
async def websocket_endpoint(
    ...
    try:
        ...
        logger.info(f"WebSocket authentication for user {user.username}, connecting to room {room_id}")
        
        # 채팅방 존재 확인
        chat_room = db.query(ChatRoom).filter(ChatRoom.id == room_id).first()
        if not chat_room:
            logger.error(f"Chat room with id {room_id} not found for WebSocket connection")
        ...
        if not is_participant:
            logger.error(f"User {user.username} tried to connect to room {room_id} but is not a participant")
            await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
            return
        
        # 웹소켓 연결 수락
        await manager.connect(websocket, room_id, user.id, user.username)
        logger.info(f"WebSocket connection established for user {user.username} in room {room_id}")
        
        try:
            while True:
                # 클라이언트로부터 메시지 수신
                data = await websocket.receive_text()
                logger.info(f"Received WebSocket message from user {user.username} in room {room_id}: {data}")
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(f"Parsed WebSocket message: type={message_type}, content={message_content}, timestamp={client_timestamp}")
                    ...
                    if not message_content or message_type != "chat":
                        ...
                        if message_type in ["typing", "read"]:
                            ...
                            logger.debug(f"Broadcasting {message_type} event from {user.username} in room {room_id}")
                            await manager.broadcast(
                                room_id=room_id,
                                message={
                                    "type": message_type,
                                    "sender_username": user.username,
                                    "timestamp": client_timestamp or datetime.now().isoformat()
                                }
                            )
                        continue
                    ...
                    if client_timestamp:
                        try:
                            ...
                            logger.warning(f"Invalid timestamp format: {client_timestamp}, Error: {str(e)}")
                            pass  # 형식이 잘못되면 무시
                    ...
                    logger.info(f"Saved and broadcasting message from {user.username} in room {room_id} (message_id: {new_message.id})")
                    await manager.broadcast(
                        room_id=room_id,
                        message={
                            "type": "chat",
                    ...
                    await manager.send_personal_message(
                        {"type": "system", "content": f"Error: {str(e)}"},
                        websocket
                    )
            ...
            if disconnect_message and room_id in manager.active_connections:
                ...
                logger.info(f"Broadcasting disconnect event for user {user.username} in room {room_id}")
