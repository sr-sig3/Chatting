...
async def send_message(
    ...
    if not message_data.content.strip():
        logger.error(f"User {current_user.username} attempted to send empty message to room {room_id}")
        raise HTTPException(
    ...
    if message_data.sender_username and message_data.sender_username != current_user.username:
        # 실제 사용자와 클라이언트가 보낸 사용자명이 다른 경우 (보안 검증)
        logger.error(f"Invalid sender username: {message_data.sender_username} (actual: {current_user.username})")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid sender username"
        )
    ...
    if message_data.timestamp:
        try:
            ...
            client_timestamp = datetime.fromisoformat(message_data.timestamp.replace('Z', '+00:00'))
    ...
    logger.info(f"Message sent to room {room_id} by user {current_user.username} (message_id: {new_message.id})")
    return MessageInfo(
        id=new_message.id,
...
async def get_messages(
    ...
    """채팅방의 메시지 목록을 조회합니다."""
    logger.info(f"Getting messages for room {room_id}. User: {current_user.username}, Page: {page}, Page size: {page_size}")
    ...
    for message in messages:
        ...
        message_infos.append(MessageInfo(
            ...
            content=message.content if not message.is_deleted else "[삭제된 메시지]",
    ...
    logger.info(f"Retrieved {len(message_infos)} messages for room {room_id} (total: {total_count})")
    return MessageList(
...
async def delete_message(
    ...
    """메시지를 삭제합니다."""
    logger.info(f"Deleting message {message_id} in room {room_id}. User: {current_user.username}")
    ...
    is_admin = db.query(ChatRoomParticipant).filter(
        and_(
            ChatRoomParticipant.chat_room_id == room_id,
            ChatRoomParticipant.user_id == current_user.id,
    ...
    if not (is_own_message or is_admin):
        logger.error(f"User {current_user.username} tried to delete message {message_id} without permission")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    logger.info(f"Message {message_id} successfully deleted by user {current_user.username}")
